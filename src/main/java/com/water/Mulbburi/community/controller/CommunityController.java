 ...
 * 
 * 
 * import com.water.Mulbburi.community.dto.CommunityDTO; import
 * com.water.Mulbburi.community.service.CommunityService;
 * 
 ...
 * 
 * @GetMapping("/regist") public String regist() { return "community/comWrite";
 * }
 * 
 ...
 * 
 * try { ======= import lombok.extern.slf4j.Slf4j;
 * 
 * @Slf4j
 * 
 * @Controller
 ...
 * @RequestMapping("/community") public class CommunityController {
 * 
 * // @Value("${image.image-dir}") // private String IMAGE_DTR; // //// private
 * final CommunityService communityService; //// private final
 * MessageSourceAccessor messageSourceAccessor; // // //
 * // @GetMapping("/regist") // public String regist() { // return
 * "community/comWrite"; // } // // // // // // // @GetMapping("/main") //
 * public String main() { // return "community/communityMain"; // }
 * 
 * >>>>>>> 050701942f800ee90f0038deb4fd3a95ff95d049
 * 
 * 4개의 input type=file이 서버로 전송 되어 List<MultipartFile> attachImage에 담겼다. for (int
 * i = 0; i < attachImage.size(); i++) { 첨부파일이 실제로 있는 경우에만 로직 수행 if
 * (attachImage.get(i).getSize() > 0) {
 * 
 ...
 * 
 * 서버의 설정 디렉토리에 파일 저장하기 attachImage.get(i).transferTo(new File(fileUploadDir +
 * "/" + savedFileName));
 * 
 * DB에 저장할 파일의 정보 처리 AttachmentDTO fileInfo = new AttachmentDTO();
 * fileInfo.setOriginalName(originalFileName);
 * fileInfo.setSavedName(savedFileName);
 * fileInfo.setSavePath("/upload/original/");
 * 
 * if(i == 0) { fileInfo.setFileType("TITLE"); 대표 사진에 대한 썸네일을 가공해서 저장한다.
 ...
 * else { fileInfo.setFileType("BODY"); } attachmentList.add(fileInfo); } }
 * 
 * log.info("[ThumbnailController] attachmentList : {}", attachmentList);
 * 
 * board.setAttachmentList(attachmentList); board.setWriter(member);
 * 
 * boardService.registThumbnail(board);
 * 
 * rttr.addFlashAttribute("message",
 * messageSourceAccessor.getMessage("board.regist"));
 * 
 * } catch (Exception e) { e.printStackTrace(); 실패 시 이미 저장 된 파일을 삭제한다.
 * for(AttachmentDTO attachment : attachmentList) {
 * 
 * File deleteFile = new File(attachment.getSavePath() + "/" +
 * attachment.getSavedName()); File deleteThumbnail = new File(thumbnailDir +
 * "/thumbnail_" + attachment.getSavedName());
 * 
 * deleteFile.delete(); deleteThumbnail.delete(); } }
 * 
 ...
 * 
 * 
 * @GetMapping("/main") public String main() { return "community/communityMain";
 * }
 * 
 * 
 * }
