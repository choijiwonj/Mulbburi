...
public class CommunityController {
	...
	public String regist() {
	  ...
	  /*디렉토리가 없을 경우 생성한다. 
	  */if(!dir1.exists() || !dir2.exists()) { dir1.mkdirs();
	  dir2.mkdirs(); }
		  ...
		  /* 4개의 input type=file이 서버로 전송 되어 List<MultipartFile> attachImage에 담겼다. */
			for (int i = 0; i < attachImage.size(); i++) {
				...
				if (attachImage.get(i).getSize() > 0) {
					...
					log.info("[CommunityController] originalFileName : {}", originalFileName);

					String ext = originalFileName.substring(originalFileName.lastIndexOf("."));
					String savedFileName = UUID.randomUUID().toString() + ext;

					log.info("[CommunityController] savedFileName : {}", savedFileName);
					...
					if(i == 0) {
						...
						Communitys.of(fileUploadDir + "/" + savedFileName).size(300, 300)
						.toFile(communityDir + "/thumbnail_" + savedFileName);
						fileInfo.setCommunityPath("/upload/thumbnail/thumbnail_" + savedFileName);
					} else {
			...
			log.info("[CommunityController] attachmentList : {}", attachmentList);
