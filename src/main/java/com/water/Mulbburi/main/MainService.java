...
public class MainService {
	...
	/*
	 * public Map<String, Object> selectMyCom(Map<String, String> searchMap, int
	 * page) { 1. 전체 게시글 수 확인 (검색어가 있는 경우 포함) => 페이징 처리 계산을 위해서 int totalCount =
	 * mainMapper.selectTotalCount(searchMap);
	 * 
	 * 한 페이지에 보여줄 게시물의 수 int limit = 4;
	 * 
	 * 한 번에 보여질 페이징 버튼의 수 int buttonAmount = 3;
	 * 
	 ...
	 * 
	 * 3. 요청 페이지와 검색 기준에 맞는 게시글을 조회해온다. List<CommunityDTO> cmList =
	 * mainMapper.selectMyCom(selectCriteria);
	 * 
	 ...
	 * 
	 * public Map<String, Object> selectMyNotice(Map<String, String> searchMap, int
	 * page) { 1. 전체 게시글 수 확인 (검색어가 있는 경우 포함) => 페이징 처리 계산을 위해서 int totalCount =
	 * mainMapper.selectTotalCount(searchMap);
	 * 
	 * 한 페이지에 보여줄 게시물의 수 int limit = 5;
	 * 
	 * 한 번에 보여질 페이징 버튼의 수 int buttonAmount = 3;
	 * 
	 * 2. 페이징 처리와 연관 된 값을 계산하여 SelectCriteria 타입의 객체에 담는다. SelectCriteria
	 ...
	 * return cmListAndPaging; }
	 * 
	 * public Map<String, Object> selectMyFAQ(Map<String, String> searchMap, int
	 * page) { 1. 전체 게시글 수 확인 (검색어가 있는 경우 포함) => 페이징 처리 계산을 위해서 int totalCount =
	 * mainMapper.selectTotalCount(searchMap);
	 * 
	 * 한 페이지에 보여줄 게시물의 수 int limit = 5;
	 ...
	 * buttonAmount, searchMap);
	 * 
	 * 3. 요청 페이지와 검색 기준에 맞는 게시글을 조회해온다. List<FaqDTO> faqList =
	 * mainMapper.selectMyFAQ(selectCriteria);
	 * 
